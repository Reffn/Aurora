...
class ReminderReconciler {
  /// Wird gerufen, wenn ein Lauf mindestens eine Erinnerung angemeldet hat.
  ///
  /// Optional und standardmaessig leer: Der Abgleich funktioniert ohne. Wer
  /// zuhoert, wird in `injection.dart` verdrahtet — der Reconciler selbst
  /// weiss nichts von Telemetrie.
  void Function()? onRemindersScheduled;
  ...
  Future<ReconcileResult> _runOnce({required int rounds}) async {
    try {
      ...
      // getragen hat — nicht, wie viele Medikamente jemand nimmt. Die Anzahl
      // waere eine Aussage ueber einen Menschen, das Gelingen eine ueber die
      // App.
      //
      // Der Reconciler kennt die Telemetrie nicht; `injection.dart` haengt
      // sich hier an, wie beim GpsManager auch.
      if (added > 0) onRemindersScheduled?.call();
