...
TELEMETRY_DOC = "/databases/(default)/documents/telemetry/doc1"

# Genau drei Felder, und der Tag ohne Uhrzeit — so sieht ein Ereignis aus,
# das TelemetryEvent.toMap() erzeugt.
TELEMETRY = {
    "event": "bereich_geoeffnet_chat",
    "day": "2026-08-04",
    "appVersion": "3.0.19",
}
...
CASES = [
    ...
    # --- Telemetrie ---------------------------------------------------------
    #
    # Bis zum 13.08.2026 prüfte diese Datei ausschließlich den Feedback-Kanal.
    # Die Telemetrie-Whitelist war damit gegen die echte Auswertung ungetestet:
    # Der Dart-Test vergleicht nur, ob Enum und Regeltext dieselben Namen
    # führen — nicht, ob die Regel sie auch durchlässt. Ein neues Ereignis wäre
    # still abgewiesen worden, und ein automatischer Sendepfad hat niemanden,
    # dem er den Fehler zeigen könnte.
    case("gültige Telemetrie anlegen", "ALLOW", "create", TELEMETRY,
         path=TELEMETRY_DOC),
    # Beweist, dass die vier neuen Ereignisse tatsächlich veröffentlicht sind.
    case("neues Ereignis: einrichtung_abgeschlossen", "ALLOW", "create",
         {**TELEMETRY, "event": "einrichtung_abgeschlossen"}, path=TELEMETRY_DOC),
    case("neues Ereignis: erinnerung_geplant", "ALLOW", "create",
         {**TELEMETRY, "event": "erinnerung_geplant"}, path=TELEMETRY_DOC),
    case("neues Ereignis: berechtigung_verweigert_benachrichtigungen", "ALLOW",
         "create", {**TELEMETRY, "event": "berechtigung_verweigert_benachrichtigungen"},
         path=TELEMETRY_DOC),
    case("neues Ereignis: app_fortgesetzt", "ALLOW", "create",
         {**TELEMETRY, "event": "app_fortgesetzt"}, path=TELEMETRY_DOC),
    # Die Whitelist ist eine Whitelist.
    case("unbekanntes Ereignis", "DENY", "create",
         {**TELEMETRY, "event": "irgendwas_neues"}, path=TELEMETRY_DOC),
    # Genau drei Felder — ein viertes wäre der Anfang einer Verkettung.
    case("Telemetrie mit viertem Feld", "DENY", "create",
         {**TELEMETRY, "profileId": "abc"}, path=TELEMETRY_DOC),
    # Der Tag trägt keine Uhrzeit: Sonst ließen sich Eingänge zu einer
    # Sitzung zusammenfassen.
    case("Tag mit Uhrzeit", "DENY", "create",
         {**TELEMETRY, "day": "2026-08-13T12:00:00Z"}, path=TELEMETRY_DOC),
    case("Telemetrie lesen", "DENY", "get", path=TELEMETRY_DOC),
    case("Telemetrie auflisten", "DENY", "list",
         path="/databases/(default)/documents/telemetry"),
]
