...
void main() {
  ...
  Future<void> legeProfilAn(
    ...
    await tester.runAsync(
      () => profileService.profilesBox.put(
        id,
        Profile(
          id: id,
          nameRaw: 'Profil $id',
  ...
  group('ResetBanner (Unit 5)', () {
    ...
    testWidgets('helle Profilfarbe hält Text und Symbole lesbar', (
      tester,
    ) async {
      await legeProfilAn(
        tester,
        'profil-aaaa-1111',
        restfrist: const Duration(hours: 5),
        farbe: Colors.white.toARGB32(),
      );

      await pumpBanner(tester);

      final foreground = AppColors.onColor(Colors.white);
      final hourglass = tester.widget<Icon>(find.byIcon(Icons.hourglass_top));
      final label = tester.widget<Text>(
        find.textContaining('Profil profil-aaaa-1111'),
      );
      final chevron = tester.widget<Icon>(find.byIcon(Icons.chevron_right));

      expect(hourglass.color, foreground);
      expect(label.style?.color, foreground);
      expect(chevron.color, foreground);
    });
    ...
    testWidgets('drei Resets stehen einzeln', (tester) async {
      ...
      await legeProfilAn(
        tester,
        'profil-bbbb-2222',
        restfrist: const Duration(hours: 3),
        farbe: 0xFF00AA00,
      );
      await legeProfilAn(
        tester,
        'profil-cccc-3333',
        restfrist: const Duration(hours: 4),
        farbe: 0xFF0000AA,
      );
