...
void main() {
  group('Farbräume', () {
    test('kein Vorschlag für Anteile kollidiert mit einer Handlungsfarbe', () {
      for (final color in AppColors.identityPalette) {
        expect(
          ...
          isFalse,
          reason: 'Identitätsfarbe ${_hex(color)} liegt zu nah an go, wait oder signal',
        );
      }
    });

    test('die drei Handlungsfarben sind für Anteile gesperrt', () {
      for (final color in <Color>[AppColors.go, AppColors.wait, AppColors.signal]) {
    ...
    test('auf jeder Identitätsfarbe bleibt der Vordergrund lesbar', () {
      ...
      const minimumContrast = 3.0; // WCAG 2.1 für große Schrift und Symbole

      for (final background in [...AppColors.identityPalette, Colors.white, Colors.black]) {
        ...
        expect(
          ...
          reason: 'Auf ${_hex(background)} erreicht der Vordergrund nur '
