...
void main() {
  ...
  // Der Fund vom 14.08.2026: Auf der Profilauswahl stand „zuletzt 6 minuti fa".
  //
  // Der Rahmen kommt aus dem Context, die Zeitangabe darin aus
  // `AppTexts.current` — einer Globalen, die die Oberflaeche beim Bauen setzt.
  // Laufen die beiden auseinander, steht ein Satz in zwei Sprachen da. Auf
  // einem einsprachigen System faellt das nie auf; in Aurora haengt die
  // Sprache am Anteil, also bei jedem Wechsel.
  //
  // Die Tests darueber haben es nicht gefunden, weil sie nur Deutsch pruefen —
  // und Deutsch ist genau der Rueckfallwert der Globalen.
  group('die Sprache kommt aus dem Aufruf, nicht aus der Globalen', () {
    testWidgets('italienischer Schirm, deutscher Rueckfall', (tester) async {
      ...
      final zeile = it.presenceLastFront(
        formatPositionAge(const Duration(minutes: 6), texts: it),
      );

      expect(zeile, contains('6 minuti fa'));
      expect(zeile, isNot(contains('Minuten')));
      expect(zeile, isNot(contains('vor ')));
    });

    test('ohne Angabe bleibt es beim bisherigen Verhalten', () {
      expect(formatPositionAge(const Duration(minutes: 7)), 'vor 7 Minuten');
    });
  });
}
